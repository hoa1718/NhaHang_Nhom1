...
function Order({props,changeTotal}) {
  ...
  const createHoaDon = ()=>{
    ...
    for(var i=0;i<props[0].length;i++){
      CTHoaDon.push(
        {"ID":props[0][i].IDMonAn,"quantity":props[0][i].quantity,"GiaBan":props[0][i].GiaBan}
      )
    }
    console.log(CTHoaDon);
    ...
    // axios.post('http://localhost:4000/ThanhToan/Create',formData,config)
    // .then(res =>{
      ...
      <div className="order-wrapper">
        ...
        <div className="purchase">
          ...
          <div style={{ display: "flex" }}>
            ...
            <button onClick={()=>{createHoaDon();console.log("hello")}} className="btn-purchase" id="done">
              THANH TOÁN
            </button>
          </div>
        </div>
