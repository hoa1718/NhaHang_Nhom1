...
function Order({props,changeTotal}) {
  ...
  const createHoaDon = (e)=>{
    ...
    for(var i=0;i<props[0].length;i++){
      CTHoaDon.push(
        {"IDMonAn":props[0][i].IDMon,"quantity":props[0][i].quantity,"GiaBan":props[0][i].GiaBan}
      )
    }
  ...
  return (
    <div className="col-lg-5">
      ...
      {id ?  <div className="desk">
      <Link to="/Ban">Bàn {}</Link>
          <i className="fas fa-solid fa-angle-right" />)
      </div> :<div className="desk">
      <Link to="/Ban">Chọn bàn</Link>
        ...
        <div className="purchase">
          ...
          <form onSubmit={(e)=>createHoaDon(e)} style={{ display: "flex" }}>
            ...
            <input type={'submit'} className="btn-purchase" id="done" value={"THANH TOÁN"}>
            </input>
          </form>
        </div>
      </div>
