...
function Order({props,changeTotal}) {
  ...
  const createHoaDon = (e)=>{
    ...
    abc.append("ID",Number(lastID))
            abc.append("IDBan",1)
            abc.append("IDKhachHang",1)
            abc.append("TrangThai",1) 
            abc.append("PhuongThuc",1)
            abc.append("NgayLap",Date.now())
            abc.append("Detail",JSON.stringify(CTHoaDon))
    const config={
      headers:{
    ...
    axios.post('http://localhost:4000/HoaDon/Create',abc,config,)
    .then(res =>{
      ...
      <div className="order-wrapper">
        ...
        <div className="purchase">
          ...
          <form onSubmit={(e)=>createHoaDon(e)} style={{ display: "flex" }}>
            ...
            <input type={'hidden'} name="IDBan" value={1}></input>
