...
function Order(props) {
  ...
  const changQuantity = (quantity,GiaBan) =>{
    let total = e.target.parentElement.nextElementSibling;
    let number = Number(e.target.value);
    total.innerText=changeMoneyFormat(number*Number(GiaBan));

  }
  const deleteFood = (i) =>{
        foodsChoose.splice(i,1);
  };
    ...
    return (
      ...
      <div className="order-item">
        ...
        <span className="item-quantity">
          ...
          <input onChange={e => changQuantity(e.target.value,foodsChoose[item].GiaBan)} className="quantity" type="text" name="quantity" min={1} defaultValue={1} />
