...
function Menu(props) {
  ...
  const getFoodsWithIDCate = (e,id) =>{
    e.preventDefault();
    async axios.get('http://localhost:4000/ThucDon/Cate/'+id)
    .then(res => {
    ...
    return Object.keys(data).map((item, i) => {
      return (
        <li key={category[item].IDPhanLoai} className="category-item">
          <div className="category-item-a" onClick={(e) =>{console.log(category[item].IDPhanLoai);getFoodsWithIDCate(e,category[item].IDPhanLoai)}} >
