...
function Menu(props) {
  ...
  console.log(props.props[2][0]);
  console.log(JSON.stringify(foods[item]) === JSON.stringify(props.props[2][i]);
  props.props[2] && console.log(props.props[2].includes(foods[1]));
  const displayFoods = (data) => {
    return Object.keys(data).map((item, i) => {
      for(let i=0; i<props.props[2].length;i++){
        if( JSON.stringify(foods[item]) === JSON.stringify(props.props[2][i]) ){
          return(
