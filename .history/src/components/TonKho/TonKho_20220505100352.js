...
function HoaDon(props) {
  ...
  const [isOpenExport, setOpenEdit] = useState(false);
  return (
    <>
      ...
      <Export open={isOpenExport} close={() => setOpenEdit(false)}></Export>
      <Import open={isOpenImport} close={() => setOpenImport(false)}></Import>
    </>
  );
}
