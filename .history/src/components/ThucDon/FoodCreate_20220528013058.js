...
function FoodCreate({ open, close }) {
  ...
  const createCongThuc =(event) =>{
    const elements = document.querySelectorAll(["classname=NguyenLieu"])
    console.log(elements);
  }
  useEffect(()=>getNguyenLieu(),createCongThuc(),[]);
  if (!open) return null;
            ...
            <div id="NguyenLieu" style={{marginLeft:"-20px"}}>
              <div className="wrap-input">
                ...
                <select
                  ...
                  onChange={(e)=>{setNguyenLieu(e.target.value);}} 
