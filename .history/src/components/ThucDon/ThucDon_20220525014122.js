...
const deleteFood =(id)=>{
  axios.delete('http://localhost:4000/ThucDon/Delete/'+id)
  .then(res => {
  })
  .catch(err =>{
    console.log(err);
        ...
        return(
                    <tr>
                      ...
                      <td>
                        ...
                        <button
                          className="btn-func"
                          onClick={() => {handleShow();setID(foods[item].IDMon);console.log(idCurrent)}}
                          style={{
                            outline: "none",
                            border: "none",
                            background: "none",
  ...
  return (
    <>
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>Bạn có chắc chắn xoá món này không ? {idCurrent}</Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" onClick={handleClose}>
            Đóng
          </Button>
          <Button variant="danger" onClick={deleteFood(idCurrent)}>
            Xoá
          </Button>
        </Modal.Footer>
      </Modal>
