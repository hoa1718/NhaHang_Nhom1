...
class ThucDon extends React.Component{
  constructor(props) {
    super(props);
  }
  state = {
  ...
  componentDidMount() {axios.get('http://localhost:4000/ThucDon')
  .then(res => {
    foods = res.data.result;
    foods.map(item=>console.log(item.TenMon))
  }).catch(err =>{
    console.log(err);
  })}
  ...
  return (
    <>
      <div style={{ position: "relative", marginBottom: "20px" }}>
        <h2 style={{  marginLeft: "25px" }}>{foods}</h2>
        <button
