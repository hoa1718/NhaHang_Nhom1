...
function FoodCreate({ open, close }) {
  ...
  const displayNL = (data) =>{
    return Object.keys(data).map((item, i) => {
      return(
          <option key={} value={datalist[item].IDNguyenLieu}>{datalist[item].TenNguyenLieu} ({datalist[item].DonViTinh})</option>
      );
    })
  }
  ...
  useEffect(()=>getNguyenLieu(),getlastID(),[]);
  if (!open) return null;
        ...
        <div className="form-create">
          ...
          <div className="wrap-input" id="CongThuc">
            ...
            <div id="NguyenLieu" style={{marginLeft:"-20px"}}>
              ...
              <div className="wrap-input">
                <button onClick={(e)=> clearNode(e)}>X</button>
              </div>
            </div>
          </div>
